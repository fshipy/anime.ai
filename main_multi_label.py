...
def train(model, optimizer, trainLoader, criterion, device, epoch, characterNames, log_interval):
    ...
    for i, data in enumerate(trainLoader, 0):
        ...
        with torch.no_grad():
            ...
            predictions = torch.zeros(TRAIN_BATCH_SIZE, len(characterNames) - include_others).long().to(device)
            confidences = nn.Sigmoid()(outputs)
            confidences[confidences < confidence_threshold] = 0.
            predict_non_others = confidences.sum(1) > 0
            if torch.any(predict_non_others):
                predictions[predict_non_others] = F.one_hot(torch.max(confidences[predict_non_others], 1)[1], len(characterNames) - include_others)

            # collect the correct predictions for each class
            for label, prediction in zip(labels, predictions):
                if torch.all(torch.eq(label, prediction)):
        ...
        if i % log_interval == log_interval - 1:    # print every 100 mini-batches
            ...
            print("===============")
        del labels, images, loss, outputs, predictions, confidences, predict_non_others # free some memory since their histories may be stored
...
def test(model, testLoader, characterNames, device):
    ...
    with torch.no_grad():
        for data in testLoader:
            ...
            predictions = torch.zeros(TEST_BATCH_SIZE, len(characterNames) - include_others).long()
            confidences = nn.Sigmoid()(outputs)
            confidences[confidences < confidence_threshold] = 0.
            predict_non_others = confidences.sum(1) > 0
            if torch.any(predict_non_others):
            ...
            del labels, images, outputs, predictions, confidences, confidences # free some memory since their histories may be stored
...
def main():
    ...
    if model_used == "alexnet":
        model = alex_net(len(labels) - include_others)
    elif model_used == "simple":
        model = SimpleAnimeNet(len(labels) - include_others)
    elif model_used == "vgg11":
        model = vgg11(len(labels) - include_others)
    elif model_used == "vgg11_bn":
        model = vgg11_bn(len(labels) - include_others) 
